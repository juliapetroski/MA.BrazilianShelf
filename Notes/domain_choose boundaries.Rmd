---
title: "Choosing StrathE2E domain extent - Brazilian Shelf"
...
For the Brazilian shelf we have inspected the distribution of vertical diffusivity values in the region from NEMO-MEDUSA model output. 60 m looks like a sensible depth to separate the shallow and deep layers.
...
Finally we need to "cut" the polygons defined by depths and distances from shore so that they don't extend around the entire landmass. For the Brazilian shelf our first suggestion is to stop in the south with the national border of Brazil, avoiding the Rio de La Plata. In the North we suggest stopping at a latitude of 22°S. If there are reasons to choose different breaks, please let us know.
...
GEBCO <- GEBCO[st_transform(st_buffer(st_union(domain), 400000), 4326)] %>%
  st_transform(crs) %>%
...
The model domain is shown in grey with white borders, while the fishing effort is shown as a heatmap on a log scale. The grey area appears to capture the majority of the fishing effort within the EEZ. There is additional fishing effort along the shelf edge, just outside the model domain, Should this be included? What is it likely to be?
