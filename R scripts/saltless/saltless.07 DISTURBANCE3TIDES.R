...
tides <- tides[v, v := i.v][                                              # Add new columns without destroying waves in memory
  coords][                                                         # Drop unwanted pixels
    ,timestep := times[timestep]]                                    # Update timestep values
...
# 
#  ggplot() +
#    geom_density(data = summary_t, aes(x=uvSpeed)) +
#    geom_density(data = all, aes(x=uvSpeed))


### If that forces movement then great, if it doesn't then there was no point working it into the depth-average.

#### Bind data and final variables ####

#all <- summary_w[summary] %>%                                                   # Fast join of wave data onto currents
all <- summary_w[summary_t] %>%                                                   # Fast join of wave data onto currents
  cbind(vectors_2_direction(.$u, .$v)) %>%                                      # Convert U V to speed and direction
  dplyr::select(-u, - v) %>% 
  mutate(D50 = case_when(Habitat == "Gravel" ~ 2.2076,                           # Attach grain sizes from Julia in mm
                         Habitat == "Sand" & Shore == "Inshore" ~ 0.228,
                         Habitat == "Sand" & Shore == "Offshore" ~ 0.3,
                         Habitat == "Silt" & Shore == "Inshore"~ 0.156,
                         Habitat == "Silt" & Shore == "Offshore"~ 0.139))

ggplot(all %>% filter(timestep == "20230101-00h")) +
  geom_raster(aes(x=x, y=y, fill = uvSpeed))

all <- filter(all, timestep %in% unique(summary_w$timestep))
...
# ggplot(data = diameter) +
#   geom_raster(aes(x, y, fill = 2^(sed_strath_diameter*-1))) +                     # Convert phi to mm 
...
stress <- shear_stress(all3$Bath, all3$sed_strath_diameter, all3$uvSpeed, all3$uvDirection, all3$swh, all3$wp, all3$mwd) %>%
  cbind(sed = all3$sed_strath_diameter)
toc()

#### Roberts package 
# all_list <- split(all3, by = c("x", "y"))
# 
# tic()
# rob <- future_map(all_list, ~{
# 
#   stress <- shear_stress(.x$Bath[[1]], 0.001, .x$uvSpeed, .x$uvDirection, .x$swh, .x$wp, .x$mwd) %>% 
#     cbind(sed = .x$D50)
# }) %>% 
#   rbindlist()
# toc()
...
disturbed <- cbind(all3, stress, movement = stress$shields_number > stress$shields_critical) %>% 
  group_by(x, y) %>% 
  summarise(movement = mean(movement))
...
mean(disturbed$movement)

#0.0069 , d50 0.001 generates some movement, For previous papers we calculated different disutrbances for each sediment fraction, then weighted by percentage.
