...
My_overhang <- readRDS("./Objects/Overhang.rds") %>% 
  .$area / sum(My_space[1:2,]$area)

#My_areas <- readRDS("./Objects/Sediment area proportions.rds") %>% 
#  mutate(Habitat = paste0(Shore, " ", Bottom)) #!!# Adjust proportions to account for overhang
...
Physical_parameters <- add_row(Physical_parameters, Value = My_overhang, .before = 13,
